...
- 요약:
  ...
  - 배틀보드 연출 애니메이션이 재생되는 동안에는 포인터 입력을 잠그고, 큐가 완전히 끝난 뒤에만 다음 포인팅을 받는다.
...
- 전투/이동 피드백 2차 보강 완료
  ...
  - swap / assist / hit / die 애니메이션 큐가 실행 중일 때는 배틀보드 포인터 입력을 잠가, 연출 도중 선택/드래그가 겹치지 않도록 고정
- 검증 상태 (최신 기준)
  - 단위 테스트: **63 passed**
...
- 배틀보드 포인터 입력은 애니메이션 큐 실행 중에는 잠그고, 큐 종료 후 다시 허용한다.
